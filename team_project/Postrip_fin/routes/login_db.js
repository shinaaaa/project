...
router.post('/', function (req, res, next) {
  ...
  function login_count() {
    return User.increment('db_count',{
      by : 1,
      where: {
  ...
  function login_check() {
    ...
    return User.findAll({
      ...
      let DB2 = JSON.stringify(result);
        ...
        console.log("alkdalksjdlajs: "+rs);
    }).catch((err) => {
      console.error("err : " + err);
      login_count();
      res.send('등록되지않은 사용자이거나 비밀번호가 틀렸습니다.');
    });
  }
  ...
  function time_check() {
    return User.findAll({
      ...
      let DB = DB1[0];
      tm = DB.db_time;
        console.log("result : time_check : " + tm);
       if(Date.now() - parseFloat(tm) > 600000) {
        count_reset();
        login();
       } else {
        res.send('잠시후 다시 시도해주세요');
       }
    }).catch((err) => {
      console.error("err : " + err);
    });
  ...
  function time_update() {
    return User.update({
    ...
    },
      {
        ...
        console.error("result : time_update : " + result);
    }).catch((err) => {
      console.error("err : " + err);
    });
  }
  ...
  function count_reset() {
    return User.update({
    ...
    },
      {
        ...
        console.log("result : count_reset : " + result);
    }).catch((err) => {
      console.error("err : " + err);
    });
  }
