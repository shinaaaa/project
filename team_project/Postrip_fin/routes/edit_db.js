...
router.post('/', upload.single('img'), function (req, res) {
  console.log("@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@ : " + (req.body.edit_email).length);
  if (req.validateErr) {
    return res.send(req.validateErr);
  }/*  else if (!req.file) {
    return res.send('<script>alert("파일을 업로드 해주세요."); history.go(-1);</script>');
  }  */else {
    if (req.body.edit_email && req.body.edit_pw) {

      if (!req.body.edit_email.match(/^[0-9a-zA-Z]([-_.]?[0-9a-zA-Z])*@[0-9a-zA-Z]([-_.]?[0-9a-zA-Z])*.[a-zA-Z]{2,3}$/i)) {
        return res.send('<script>alert("이메일 형식에 맞지않습니다."); history.go(-1);</script>');
      } else {

        if((req.body.edit_email).length > 20) {
          return res.send('<script>alert("이메일을 20자 이내로 입력해주세요."); history.go(-1);</script>');
        } else if((req.body.edit_pw).length > 255) {
  ...
  function update() {
    console.log("12312323123123123123 : " + req.session.image);
    User.update({
      db_email: req.body.edit_email,
      db_pw: SHA256(req.body.edit_pw),
      db_image: req.session.image
    },
      {
        where: {
      ...
      .catch((err) => {
        ...
        res.send("<script>alert('이미 사용중인 이메일입니다. 다시 입력해주세요.'); history.go(-1);</script>");
      });
  }

  function pw_confirm() {

    var pw = req.body.edit_pw;
  ...
  function email_confirm2() {
    ...
    return User.findAll({
      ...
      rs = DB.db_email;
      if (rs == req.body.edit_email) {
        console.log("id_confirm123 : false : " + result);
        //res.end();
        rs = false;
        console.log("찍어본다123 : " + rs);
        res.redirect('/mypage2');
        return rs;
      } else {
        console.log("id_confirm : true : " + result);
