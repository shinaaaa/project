...
router.post('/', function (req, res, next) {
  console.log("@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@");
  console.log("@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@ : req.body.pw_auth : " + req.body.pw_auth);
    if(req.body.pw_auth == "") {
      res.send("비밀번호를 입력해주세요.");
    } else {
      User.findAll({
        attributes: [ 'db_email','db_id', 'db_name','db_birth', 'db_image' ],
        where: {
          db_id: req.session.lid,
          db_pw: SHA256(req.body.pw_auth)
        }
      }).then((result) => {
        ...
        res.send("비밀번호가 틀렸습니다. 다시 입력해주세요.");
      });
    }
